...
const getEcartsCount = (e) => {
  if (!e) return 0;
  return (e.petit || 0) + (e.moyen || 0) + (e.gros || 0);
};

const getEcartsKcal = (e) => {
  if (!e) return 0;
  return ((e.petit || 0) * 300) + ((e.moyen || 0) * 600) + ((e.gros || 0) * 1000);
};
...
const calcBMR = (profile) => {
  ...
  const p = profile.poids || 75;
  const t = profile.taille || 175;
  const a = profile.age || 30;
  if (profile.sexe === 'homme') return Math.round(10 * p + 6.25 * t - 5 * a + 5);
  return Math.round(10 * p + 6.25 * t - 5 * a - 161);
};
...
export default function CoachZen() {
  ...
  const realToday = useMemo(() => {
    try { return formatDate(new Date()); } catch { return '2025-01-05'; }
  }, []);
  ...
  useEffect(() => {
    if (mounted && selectedDate && allData) {
  ...
  const saveWeight = (weight) => {
    try {
      const today = formatDate(new Date());
      const filtered = weightHistory.filter(w => w && w.date !== today);
      const newHistory = [...filtered, { date: today, weight: Number(weight) }];
  ...
  const totalKcal = Object.entries(MEALS).reduce((sum, [k, m]) => {
    return (dayData && dayData.habits && dayData.habits[k]) ? sum + m.kcal : sum;
  }, 0);
  const bmr = calcBMR(profile);
  const tdee = calcTDEE(bmr, profile ? profile.activite : 'modere');
  ...
  const updateMovement = (k, v) => {
    setDayData(p => ({ 
      ...p, 
      movement: { ...(p.movement || {}), [k]: v } 
    }));
  };
  ...
  const streak = useMemo(() => { 
    try {
      ...
      for (let i = 1; i <= 30; i++) { 
        ...
        if (allData && allData[key] && calcScore(allData[key]) >= 50) s++; 
        else break; 
      } 
      return s;
    } catch {
  ...
  const last7Days = useMemo(() => {
    try {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        days.push({ 
          date: key, 
          score: (allData && allData[key]) ? calcScore(allData[key]) : 0, 
      ...
      for (let w = 0; w < 4; w++) {
        ...
        for (let d = 0; d < 7; d++) {
          ...
          if (allData && allData[key]) {
            weekScores.push(calcScore(allData[key]));
          }
        }
        const avg = weekScores.length > 0 ? Math.round(weekScores.reduce((a, b) => a + b, 0) / weekScores.length) : 0;
        weeks.unshift({ label: 'S' + (4-w), score: avg });
      }
      return weeks;
    } catch {
      return [{ label: 'S1', score: 0 }, { label: 'S2', score: 0 }, { label: 'S3', score: 0 }, { label: 'S4', score: 0 }];
    }
  }, [allData]);
  ...
  const monthAvg = useMemo(() => { 
    try {
      ...
      const scores = values.slice(-30).map(d => calcScore(d)).filter(s => s > 0); 
  ...
  let selectedDateObj;
  try {
    selectedDateObj = selectedDate ? new Date(selectedDate + 'T12:00:00') : new Date();
  } catch {
    ...
    <div style={container}>
      <div style={content}>
        ...
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, paddingTop: 8 }}>
          ...
          <button onClick={() => { setShowAnalysis(true); fetchAnalysis('week'); }} style={{ background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', border: 'none', borderRadius: 10, padding: '8px 14px', cursor: 'pointer' }}>
        ...
        {tab === 'today' && (
          <>
              ...
              <div style={{ background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', padding: 2, borderRadius: 16, marginBottom: 12 }}>
                <div style={{ background: 'rgba(15,23,42,0.95)', borderRadius: 14, padding: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 10, background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ color: 'white', fontWeight: 'bold', fontSize: 14 }}>Z</span>
                    </div>
            ...
            <div style={{ ...card, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16 }}>
              ...
              <div style={{ width: 80, height: 80, borderRadius: 40, border: '6px solid #8b5cf6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: 28, fontWeight: 'bold', color: 'white' }}>{score}</span>
              </div>
            </div>
            ...
            <div style={{ ...card }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 16, fontWeight: 'bold', color: totalKcal + getEcartsKcal(dayData.ecarts) > tdee ? '#ef4444' : '#10b981' }}>
                  {totalKcal + getEcartsKcal(dayData.ecarts)} / {tdee}
                </span>
              </div>
              <div style={{ height: 8, background: 'rgba(255,255,255,0.1)', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ height: '100%', background: '#10b981', width: `${Math.min(((totalKcal + getEcartsKcal(dayData.ecarts)) / tdee) * 100, 100)}%` }} />
              </div>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', margin: '8px 0 0', textAlign: 'center' }}>
                {(tdee - totalKcal - getEcartsKcal(dayData.ecarts)) > 0 
                  ? `📉 Déficit: −${tdee - totalKcal - getEcartsKcal(dayData.ecarts)} kcal` 
                  : `📈 Surplus: +${Math.abs(tdee - totalKcal - getEcartsKcal(dayData.ecarts))} kcal`}
              </p>
            </div>

            {/* Meals */}
            {Object.entries(MEALS).map(([k, m]) => {
              const checked = dayData && dayData.habits && dayData.habits[k];
              return (
                  ...
                  <div style={{ background: checked ? 'rgba(255,255,255,0.15)' : 'rgba(15,23,42,0.95)', borderRadius: 15, padding: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ flex: 1, textAlign: 'left' }}>
                        <p style={{ fontSize: 16, fontWeight: 'bold', color: 'white', margin: 0 }}>{m.title}</p>
                        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', margin: 0 }}>{m.time} • {m.kcal} kcal</p>
                      </div>
                      <div style={{ padding: '6px 12px', borderRadius: 10, background: 'rgba(255,255,255,0.1)' }}>
                        <span style={{ fontSize: 14, fontWeight: 'bold', color: 'white' }}>{checked ? '✓' : '+20'}</span>
                      </div>
                    </div>
                  </div>
                </button>
            ...
            <div style={{ ...card, background: 'linear-gradient(135deg, rgba(239,68,68,0.1), rgba(249,115,22,0.1))', border: '1px solid rgba(239,68,68,0.2)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {ECARTS.map(e => {
                  const count = (dayData && dayData.ecarts && dayData.ecarts[e.id]) || 0;
                  return (
            ...
            <div style={card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span style={{ color: 'rgba(255,255,255,0.6)' }}>🌙 Sommeil</span>
                <span style={{ fontWeight: 'bold' }}>{dayData.sleep || 7}h</span>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, marginTop: 14 }}>
                <span style={{ color: 'rgba(255,255,255,0.6)' }}>☀️ Sieste</span>
                <span style={{ fontWeight: 'bold' }}>{dayData.nap || 0} min</span>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 }}>
              {[
                { key: 'workout', emoji: '🏋️', label: 'Muscu', color: '#ec4899' },
                { key: 'run', emoji: '🏃', label: 'Course', color: '#f59e0b' },
                { key: 'walk', emoji: '🚶', label: 'Marche', color: '#06b6d4' }
              ].map(m => {
                const active = dayData && dayData.movement && dayData.movement[m.key];
                return (
                  <button key={m.key} onClick={() => updateMovement(m.key, !active)} style={{ padding: 14, borderRadius: 12, border: 'none', cursor: 'pointer', background: active ? m.color : 'rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    <span style={{ fontSize: 24 }}>{m.emoji}</span>
                    <span style={{ color: active ? 'white' : 'rgba(255,255,255,0.5)', fontSize: 12 }}>{m.label}</span>
                  </button>
                );
              })}
            </div>
        ...
        {tab === 'week' && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
              <div style={{ background: '#8b5cf6', borderRadius: 16, padding: 16, textAlign: 'center' }}>
                <p style={{ fontSize: 32, fontWeight: 'bold', margin: 0 }}>{streak}</p>
                <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 12 }}>🔥 streak</p>
              </div>
              <div style={{ background: '#06b6d4', borderRadius: 16, padding: 16, textAlign: 'center' }}>
                <p style={{ fontSize: 32, fontWeight: 'bold', margin: 0 }}>{monthAvg}</p>
                <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 12 }}>moy/30j</p>
              </div>
            </div>
            <div style={card}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4 }}>
                {last14Days.map((d, i) => {
                  ...
                  const data = allData ? allData[k] : null;
          ...
          <>
            ...
            <div style={{ ...card }}>
              <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14, margin: 0 }}>
                Total: <strong style={{ color: '#10b981' }}>2650 kcal</strong> • TDEE: <strong style={{ color: '#06b6d4' }}>{tdee} kcal</strong>
              </p>
            </div>
            {Object.entries(MEALS).map(([k, m]) => (
              <div key={k} style={{ background: `linear-gradient(135deg, ${m.colors[0]}, ${m.colors[1]})`, borderRadius: 16, padding: 14, marginBottom: 10 }}>
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: 12, margin: 0, opacity: 0.8 }}>{m.time} • {m.kcal} kcal</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        {tab === 'stats' && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 14 }}>
              <div style={{ background: '#10b981', borderRadius: 14, padding: 12, textAlign: 'center' }}>
                <p style={{ fontSize: 26, fontWeight: 'bold', margin: 0 }}>{totalDays}</p>
                <p style={{ fontSize: 11, margin: 0, opacity: 0.8 }}>jours</p>
              </div>
              <div style={{ background: '#8b5cf6', borderRadius: 14, padding: 12, textAlign: 'center' }}>
                <p style={{ fontSize: 26, fontWeight: 'bold', margin: 0 }}>{streak}</p>
                <p style={{ fontSize: 11, margin: 0, opacity: 0.8 }}>🔥 streak</p>
              </div>
              <div style={{ background: '#f59e0b', borderRadius: 14, padding: 12, textAlign: 'center' }}>
                <p style={{ fontSize: 26, fontWeight: 'bold', margin: 0 }}>{monthAvg}</p>
                <p style={{ fontSize: 11, margin: 0, opacity: 0.8 }}>moy</p>
              </div>
            </div>

            {/* Weight */}
            <div style={card}>
              <p style={{ fontSize: 12, fontWeight: 'bold', color: 'rgba(255,255,255,0.6)', margin: '0 0 10px' }}>⚖️ POIDS</p>
              {weightHistory.length > 0 ? (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 24, fontWeight: 'bold' }}>{weightHistory[weightHistory.length - 1].weight} kg</span>
                  <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>{weightHistory.length} pesée(s)</span>
                </div>
              ) : (
                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 14, margin: 0 }}>Aucune pesée</p>
              )}
            </div>

            <button onClick={() => { setModalWeight(profile.poids || 75); setShowWeightModal(true); }} style={{ width: '100%', padding: 14, borderRadius: 12, border: 'none', cursor: 'pointer', background: '#06b6d4', marginBottom: 14 }}>
              <span style={{ color: 'white', fontSize: 14, fontWeight: 'bold' }}>⚖️ Enregistrer mon poids</span>
            </button>

            {/* 7 days chart */}
            <div style={card}>
              <p style={{ fontSize: 12, fontWeight: 'bold', color: 'rgba(255,255,255,0.6)', margin: '0 0 10px' }}>📈 7 DERNIERS JOURS</p>
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 60 }}>
                {last7Days.map((d, i) => (
                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    <div style={{ width: '100%', height: Math.max(4, d.score * 0.5), background: d.score >= 80 ? '#10b981' : d.score >= 50 ? '#f59e0b' : d.score > 0 ? '#ef4444' : 'rgba(255,255,255,0.1)', borderRadius: 4 }} />
            ...
            <div style={card}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <label style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>Poids</label>
                  <input type="number" value={profile.poids || 75} onChange={e => setProfile(p => ({ ...p, poids: Number(e.target.value) || 75 }))} style={{ width: '100%', padding: 10, borderRadius: 8, border: 'none', background: 'rgba(255,255,255,0.1)', color: 'white', marginTop: 4, boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>Taille</label>
                  <input type="number" value={profile.taille || 175} onChange={e => setProfile(p => ({ ...p, taille: Number(e.target.value) || 175 }))} style={{ width: '100%', padding: 10, borderRadius: 8, border: 'none', background: 'rgba(255,255,255,0.1)', color: 'white', marginTop: 4, boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>Âge</label>
                  <input type="number" value={profile.age || 30} onChange={e => setProfile(p => ({ ...p, age: Number(e.target.value) || 30 }))} style={{ width: '100%', padding: 10, borderRadius: 8, border: 'none', background: 'rgba(255,255,255,0.1)', color: 'white', marginTop: 4, boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>Sexe</label>
                  <select value={profile.sexe || 'homme'} onChange={e => setProfile(p => ({ ...p, sexe: e.target.value }))} style={{ width: '100%', padding: 10, borderRadius: 8, border: 'none', background: 'rgba(255,255,255,0.1)', color: 'white', marginTop: 4 }}>
              ...
              <div style={{ marginTop: 10 }}>
                <label style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>Activité</label>
                <select value={profile.activite || 'modere'} onChange={e => setProfile(p => ({ ...p, activite: e.target.value }))} style={{ width: '100%', padding: 10, borderRadius: 8, border: 'none', background: 'rgba(255,255,255,0.1)', color: 'white', marginTop: 4 }}>
            ...
            {/* Metabolism */}
            <div style={card}>
              <p style={{ fontSize: 12, fontWeight: 'bold', color: 'rgba(255,255,255,0.6)', margin: '0 0 10px' }}>🔥 MÉTABOLISME</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: 12, padding: 12, textAlign: 'center' }}>
                  ...
                  <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', margin: 0 }}>BMR</p>
                </div>
                <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: 12, padding: 12, textAlign: 'center' }}>
                  <p style={{ fontSize: 24, fontWeight: 'bold', margin: 0, color: '#10b981' }}>{tdee}</p>
                  <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', margin: 0 }}>TDEE</p>
                </div>
              </div>
            </div>
          </>
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'rgba(15,23,42,0.98)', borderTop: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-around', padding: '8px 0 24px', zIndex: 9999 }}>
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', padding: 6 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', background: tab === t.id ? '#8b5cf6' : 'transparent' }}>
          ...
          <div style={{ background: '#1e293b', borderRadius: 20, padding: 20, maxWidth: 320, width: '100%' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginBottom: 20 }}>
              ...
              <div style={{ textAlign: 'center' }}>
                <span style={{ fontSize: 40, fontWeight: 'bold', color: 'white' }}>{modalWeight.toFixed(1)}</span>
                <span style={{ fontSize: 18, color: 'rgba(255,255,255,0.5)', marginLeft: 4 }}>kg</span>
              </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowWeightModal(false)} style={{ flex: 1, padding: 14, borderRadius: 12, border: 'none', background: 'rgba(255,255,255,0.1)', color: 'white', fontSize: 14, cursor: 'pointer' }}>Annuler</button>
              <button onClick={() => saveWeight(modalWeight)} style={{ flex: 1, padding: 14, borderRadius: 12, border: 'none', background: '#06b6d4', color: 'white', fontSize: 14, fontWeight: 'bold', cursor: 'pointer' }}>Enregistrer</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setShowAnalysis(false)}>
          <div style={{ background: '#1e293b', borderRadius: 20, padding: 20, maxWidth: 380, width: '100%', maxHeight: '80vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h2 style={{ fontSize: 20, fontWeight: 'bold', margin: 0, color: 'white' }}>🤖 Analyse IA</h2>
              <button onClick={() => setShowAnalysis(false)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: 24, cursor: 'pointer' }}>×</button>
            </div>
            <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
              <button onClick={() => fetchAnalysis('week')} style={{ flex: 1, padding: 12, borderRadius: 10, border: 'none', cursor: 'pointer', background: analysisPeriod === 'week' ? '#8b5cf6' : 'rgba(255,255,255,0.1)', color: 'white', fontWeight: 'bold' }}>7 jours</button>
              <button onClick={() => fetchAnalysis('month')} style={{ flex: 1, padding: 12, borderRadius: 10, border: 'none', cursor: 'pointer', background: analysisPeriod === 'month' ? '#8b5cf6' : 'rgba(255,255,255,0.1)', color: 'white', fontWeight: 'bold' }}>30 jours</button>
            </div>
            {analysisLoading ? (
              <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.5)' }}>Analyse en cours...</p>
            ) : (
